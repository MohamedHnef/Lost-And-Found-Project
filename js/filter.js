...
function initDataFetching() {
    ...
    .then(([lostItems, foundItems]) => {
        originalData = [...lostItems, ...foundItems];
...
function displayItems(data) {
    ...
    cardLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            localStorage.setItem('selectedItemStatus', itemStatus);
...
function createCard(item) {
    ...
    const formattedTime = formatTime(item.foundTime || item.timeLost);
...
function populateFilters(data) {
    const locations = [...new Set(data.map(item => item.locationLost || item.locationFound))];
...
function applyFilters() {
    ...
    const filteredData = originalData.filter(item =>
        (!fromDate || new Date(item.lostDate || item.foundDate) >= new Date(fromDate)) &&
        (!toDate || new Date(item.lostDate || item.foundDate) <= new Date(toDate)) &&
        (!location || item.locationLost === location || item.locationFound === location) &&
