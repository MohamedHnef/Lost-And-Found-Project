...
router.post('/item-found', (req, res) => {
  ...
  console.log('Received request to send notification email for item found with ID:', itemId);
  ...
  pool.query(query, [itemId], (err, results) => {
    ...
    if (results.length === 0) {
      console.log('No matching item found in the database for the provided item ID:', itemId);
      return res.status(404).send('No matching item found');
    }

    const { lostContactEmail, lostUsername, itemName } = results[0];
    console.log(`Sending email to: ${lostContactEmail} for item: ${itemName}`);
    ...
    // Send email to the contactEmail of the user who reported the item as lost
